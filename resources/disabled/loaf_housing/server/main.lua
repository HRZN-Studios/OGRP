...
CreateThread(function()
    ...
    function GiveProperty(source, cb, propertyId, purchaseOrRent, shellId)
        ...
        elseif housedata.locations then
            ...
            for k, v in pairs(locations) do
                if v.storage then
                    RegisterStash(propertyId, uniqueId, k, Config.DefaultSlots or 50, Config.DefaultWeight or 25000, GetIdentifier(source))
                end
            end
        end
    ...
    lib.RegisterCallback("loaf_housing:transfer_property", TransferProperty)

    lib.RegisterCallback("loaf_housing:house_exists", function(source, cb, propertyId, uniqueId)
        cb(MySQL.Sync.fetchScalar("SELECT `owner` FROM `loaf_properties` WHERE `propertyId`=@propertyId AND `id`=@uniqueId", {
            ["@propertyId"] = propertyId,
            ["@uniqueId"] = uniqueId
        }) ~= nil)
    end)
