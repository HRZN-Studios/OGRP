...
elseif Config.Frameworks.QB.enabled then
    ...
    print('[^2INFO] ^0[^5QF-MDT-EMS^0] encountered a problem at [^5Server Framework Export^0], if you are using a [^5CUSTOM FRAMEWORK / STANDALONE^0] open a ticket on our discord server and wait for help with configuration!')
...
FRAMEWORK.GetPlayerFromId = function(id)
    ...
    if Config.Frameworks.ESX.enabled then
        ...
        if xPlayer then
            ...
            data.showNotification = function(msg)
                return TriggerClientEvent('qf-mdt-ems:showNotification', id, msg)
            end
    ...
    elseif Config.Frameworks.QB.enabled then
        ...
        if xPlayer then
            ...
            data.showNotification = function(msg)
                TriggerClientEvent('qf-mdt-ems:showNotification', id, msg)
            end
            data.getIdentifier = function ()
        ...
        print('[^2INFO] ^0[^5QF-MDT-EMS^0] encountered a problem at [^5GetPlayerFromId^0], if you are using a [^5CUSTOM FRAMEWORK / STANDALONE^0] open a ticket on our discord server and wait for help with configuration!')
        -- OR SCRIPT IT BY OWN
    end
...
FRAMEWORK.GetPlayerFromIdentifier = function(identifier)
    ...
    elseif Config.Frameworks.QB.enabled then
        ...
        print('[^2INFO] ^0[^5QF-MDT-EMS^0] encountered a problem at [^5GetPlayerFromIdentifier^0], if you are using a [^5CUSTOM FRAMEWORK / STANDALONE^0] open a ticket on our discord server and wait for help with configuration!')
        -- SCRIPT IT BY OWN
    end
end
...
FRAMEWORK.GetPlayers = function()
    ...
    elseif Config.Frameworks.QB.enabled then
        ...
        print('[^2INFO] ^0[^5QF-MDT-EMS^0] encountered a problem at [^5GetPlayers^0], if you are using a [^5CUSTOM FRAMEWORK / STANDALONE^0] open a ticket on our discord server and wait for help with configuration!')
...
FRAMEWORK.GetFirstName = function(player)
    ...
    elseif Config.Frameworks.QB.enabled then
        ...
        print('[^2INFO] ^0[^5qf-mdt-lsc^0] encountered a problem at [^5GetFirstName^0], if you are using a [^5CUSTOM FRAMEWORK / STANDALONE^0] open a ticket on our discord server and wait for help with configuration!')
        return "John"
        -- SCRIPT IT BY OWN
    end
end
...
FRAMEWORK.GetLastName = function(player)
    ...
    elseif Config.Frameworks.QB.enabled then
        ...
        print('[^2INFO] ^0[^5qf-mdt-lsc^0] encountered a problem at [^5GetLastName^0], if you are using a [^5CUSTOM FRAMEWORK / STANDALONE^0] open a ticket on our discord server and wait for help with configuration!')
        return "Smith"
        -- SCRIPT IT BY OWN
    end
end
