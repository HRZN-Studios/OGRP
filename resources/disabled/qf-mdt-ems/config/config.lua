...
Config.Language = 'pl' --// Supported languages ['en' - English/US | 'fr' - French | 'pl' - Polish | 'de' - Deutsch | 'es' - Spanish]

Config.Society = { --// Society name, percents configs to receive after sending to giving a invoices
    name = "ambulance",
    jobname = "ambulance",
    label = 'EMS',
    percentToHospital = 0.80,
    percentToDoctor = 0.20,
}
...
Config.MaxDistanceToInvoice = 15 --// Maximum distance for send player give invoices
Config.ShowRadio = true --// IF you want to radio channel will be seen in doctorlist
...
Config.Jobs = {
    ...
    OnDuty = {
        ['ambulance'] = true, --// Job name to access to MDT
    },
    KickDuty = {
        name = 'offambulance', --// Job name to set after kick from duty
        Grade = {
            grade = 0, --// Grade number to set after kick from duty if you are using option "set_grade = true"
            set_grade = false, --// Set new grade to player
            same_grade = true, --// Set same grade to player what have
        }
    },
...
Config.ToggleMDT = {
    ...
    commandName = 'mdtems', --// Command name
    keymappingLabel = 'Open Tablet [EMS]' --// Command Label in Keymapping
}
...
Config.Dispatch = {
    ...
    notif_dispatch = true, -- // TriggerClientEvent('qf-mdt-ems:addDispatchAlert', -1, coords, 'title', 'subtitle', 'code f.e. 10-90', 'rgb(255, 0, 255)', '10 - max number of people that can response if you dont want it, set it to 0')
...
Config.Sounds = {
    ...
    Custom = { -- // Custom sound from your file with .ogg extension from sounds folder
        use = true,
        triggerName = "interact-sound_CL:PlayOnSource", --//InteractSound event name
...
Config.CitizenLicenses = {
    {
        ...
        licenseIcon = 'fa-solid fa-notes-medical', -- search here => https://fontawesome.com/icons // If "isText" is true set it to => ""
    },
}
