...
elseif Config.Framework == "ESX" then
    Config.PrimaryIdentifier = Core?.GetConfig().Identifier or GetConvar("sv_lan", "") == "true" and "ip" or "license"

    local ESXConfigPath = GetResourcePath('es_extended') .. '/config.lua'
    local file = io.open(ESXConfigPath, "r")

    if not file then
        Functions.Print("Cannot open your es_extended config file. Please make sure that Config.Multichar in your es_extended is set to true")
        return
    end
    ...
    if not content then
        Functions.Print("Cannot read your es_extended config file. Please make sure that Config.Multichar in your es_extended is set to true")
        return
    end
    ...
    if multicharExists then
        ...
        if currentMulticharValue ~= "true" then
            local updatedContent = string.gsub(content, '(Config.Multichar%s*=%s*)[^%s]+', '%1true')
            local file = io.open(ESXConfigPath, "w")
            ...
            file:close()
            Functions.Print("Automatically changed your es_extended/config.lua:Config.Multichar to true as it's required")
        end
    else
        Functions.Print("Config.Multichar not found in your es_extended config file. Please make sure that Config.Multichar in your es_extended is set to true")
    end
end
...
function FetchDiscordRoles(memberId)
    ...
    PerformHttpRequest(string.format("https://discord.com/api/v10/guilds/%s/members/%s", Selector.Discord.Guild, memberId), function(status, body)
        if status ~= 200 then
            promise:resolve(nil)
            return
        end
...
Selector.GetMaxSlots = function(src)
    ...
    if MySQL.scalar.await("SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'multicharacter_slots' AND table_schema = DATABASE()") > 0 then
        ---@diagnostic disable-next-line: undefined-global
        maxCharacters = MySQL.scalar.await('SELECT slots FROM multicharacter_slots WHERE identifier = ?', { identifier }) or maxCharacters
    end
